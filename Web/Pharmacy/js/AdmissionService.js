...
var AdmissionServiceViewModel = (function(){
    var model = function (http, warningMessageService, rootScope, commonModalService) {
        ...
        self.UploadPhoto = function(data){
            ...
            http.post('/api/PharmacyUploadPhoto', formData, {
        ...
        self.CheckSeatsAvailbility = function () {
            http({
                method: 'GET',
                url: '/api/PharmacyCheckSeats?Quota=' + self.Admission.Quota + '&BranchId=' + self.Admission.BranchId
            })
            .success(function (data) {
                if (data) {                    
                    self.AdmitStudent();
        ...
        self.ShowMessage = function (title,content) {
            warningMessageService.OpenPopup({ template: '/Pharmacy/Templates/CommonModalTemplate.html', controller: 'CommonModalController', Heading:title, Body : content});
        };
    };
