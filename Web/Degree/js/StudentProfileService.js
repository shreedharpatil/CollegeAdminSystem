...
var StudentProfileServiceViewModel = (function(){
    var model = function(http,rootScope,warningMessageService){
        ...
        self.Years = ['First Sem', 'Second Sem', 'Third Sem', 'Fourth Sem', 'Fifth Sem', 'Sixth Sem'];
        ...
        self.LoadStudentFeeDetails = function(){
            http({
                method:'GET',
                url: '/api/Degreestudent?id=' + self.Student.Id
            })
                .success(function(data){
                    self.AdmissionDetails = data;
                    rootScope.studentProfileDetails.Year = data.length +1;//self.Years[data.length];
                    self.TotalFee = 0;
                    self.TotalDueFee = 0;
                    self.TotalPaidFee = 0;
        ...
        self.NewAdmission = function(){
            warningMessageService.OpenPopup({ template: '/Degree/Templates/AddFeeDetailsModalTemplate.html', controller: 'AddFeeDetailsModalController', Callback: self.LoadStudentFeeDetails });
        };
        ...
        self.UpdateFeeDetails = function(fee){
            ...
            warningMessageService.OpenPopup({ template: '/Degree/Templates/EditFeeDetailsModalTemplate.html', controller: 'EditFeeDetailsModalController', Callback: self.LoadStudentFeeDetails });
        };
    };
