...
{
    public class GenerateDiplomaFeeReceiptQuery : IQueryFor<Student, MemoryStream>
    {
        public MemoryStream ExecuteQueryWith(Student student)
        {
            ...
            var p1 = new PdfPCell(new Phrase("Ph:08473-250600", FontFactory.GetFont("Arial", 12)));
            var pp1 = new PdfPCell(new Phrase("College Code:514", FontFactory.GetFont("Arial", 12)));
            p1.HorizontalAlignment = 0;
            pp1.HorizontalAlignment = 2;
            p1.BorderWidth = 0;
            pp1.BorderWidth = 0;
            table1.AddCell(p1);
            table1.AddCell(pp1);

            var p2 = new PdfPCell(new Phrase("NIVEDITA POLYTECHNIC TRUST(R)", FontFactory.GetFont("Arial", 12)));
            BaseFont bfTimes = BaseFont.CreateFont(BaseFont.TIMES_ROMAN, BaseFont.CP1252, false);
            var times = new iTextSharp.text.Font(bfTimes, 15, iTextSharp.text.Font.BOLD);
            var p3 = new PdfPCell(new Phrase("NIVEDITA POLYTECHNIC", times));

            var p4 = new PdfPCell(new Phrase("(Affiliated to BTE  Bangalore & approved by A.I.C.T.E, New Delhi)", FontFactory.GetFont("Arial", 12)));
            ...
            var s1 = new PdfPCell(new Phrase("No. 563", GetFont(12f)));
            var s2 = new PdfPCell(new Phrase("Receipt", GetFont(12f)));
            var s3 = new PdfPCell(new Phrase("Date:" + student.DateOfAdmission.FormatDate(), GetFont(12f)));
            s2.HorizontalAlignment = 1;
            secondRow.AddCell(s1);
            secondRow.AddCell(s2);
            secondRow.AddCell(s3);
            ...
            var p7 = new Phrase("Father's Name: " + student.FatherName, FontFactory.GetFont("Arial", 12));
            var p8 = new Phrase("Course: " + student.BranchName, FontFactory.GetFont("Arial", 12));
            var p9 = new Phrase("Year: " + student.Year, FontFactory.GetFont("Arial", 12));
            ...
            IList<FeeStructure> fees = new List<FeeStructure> { 
                new FeeStructure{ Description = "Admission Form", Amount = student.AdmissionFormFee },new FeeStructure{ Description = "Admission Fee", Amount = student.AdmissionFee },
                new FeeStructure{ Description = "Tution Fee", Amount = student.CourseFee },new FeeStructure{ Description = "Identity Card Fee", Amount = student.IdentityCardFee },
                new FeeStructure{ Description = "Student's Association Fee", Amount = student.StudentAssociationFee },new FeeStructure{ Description = "Reading Room Fee", Amount = student.ReadingRoomFee },
                new FeeStructure{ Description = "Magazine Fee", Amount = student.MagazineFee },new FeeStructure{ Description = "Library Fee", Amount = student.LibraryFee },
                new FeeStructure{ Description = "Laboratory Fee", Amount = student.LabFee },new FeeStructure{ Description = "Student's Welfare Fund(SWF)", Amount = student.StudentWelfareFundFee },
                new FeeStructure{ Description = "Teacher's Benefit Fund(TBF)", Amount = student.TeacherBenefitFundFee },new FeeStructure{ Description = "Other Fee", Amount = student.OtherFee },
                new FeeStructure{ Description = "Admission Fine", Amount = student.AdmissionFineFee },new FeeStructure{ Description = "Sports Fee", Amount = student.SportsFee },
                new FeeStructure{ Description = "Fine", Amount = student.FineFee },new FeeStructure{ Description = "", Amount = 0 },
                new FeeStructure{ Description = "Total Fee Rs.", Amount = student.TotalFee },new FeeStructure{ Description = "Total Fee Paid Rs.", Amount = student.PaidFee },
                new FeeStructure{ Description = "Due Fee Rs.", Amount = student.DueFee }
            };
            ...
            var c5 = new PdfPCell(new Phrase("Note: Please keep this receipt as record and show on demand. Fees once paid will not be refunded.", FontFactory.GetFont("Arial", 12)));
